...
namespace Database{
	...
	private:
		...
		// Opaque binary-compatible storage for stdext::hash_map<std::string, int>
		// as used in the VS2008-compiled CvGameDatabaseWin32 DLL.
		// All methods that access this field are __declspec(dllimport) and execute
		// within the VS2008 DLL context, so the layout is correct there.
		// 64 bytes is safely larger than VS2008's stdext::hash_map internal size
		// (~28-36 bytes), ensuring VS2013's sizeof(Results) >= VS2008's sizeof(Results)
		// and preventing stack/heap buffer overruns when the DLL initializes the field.
		__declspec(align(8)) char m_hshColumnPositions[64];
	};
